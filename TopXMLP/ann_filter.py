...
def run_ann_filter(json_file):
    ...
    for entry in data:
        ...
        input_text = []
        output_text = []
        text_features = [1]     #reputacao do autor = 1
        ...
        if(len(input_text) > 1):
            print(entry['revisao'])
            annManager.AnnTraining(input_text+input_text, output_text+output_text, 2)
            


if __name__ == '__main__':
    run_ann_filter('reviews.json')
