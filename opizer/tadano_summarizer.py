...
class Tadano_Summarizer(object):
    ...
    def __init__(self, review_data, review_key, id_key, aspect_manager):
        ...
        self.__clusters = {}
        self.__aspect_list = {key:0 for key in aspect_manager.get_aspects_reviews()}
    ...
    def __read_files(self, review_data, review_key, id_key):
        ...
        for review in review_data:
            ...
            stars = self.__aspect_manager.get_stars_review( str(review[id_key]) )
            
            review_words = []
            for (id_sentence, text_sentence) in sentences:
                id_general = id_sentence + "_" + str(review[id_key])
            ...
            for word in review_words:
                ...
                review_id = 'review_' + str(review[id_key])
                if review_id not in all_words[word]: all_words[word].append(review_id)

        self.__calculate_tfidf(float(len(review_data)), all_words)
    ...
    def __iterate_aspects(self, vectors, id_sentences):
        ...
        for aspect in self.__aspect_list.keys():
            ...
            for index, id_sentence in enumerate(id_sentences):
                ...
                if aspect in self.__aspect_manager.get_aspects_sentence(match.group(2), match.group(1)):
    ...
    def __kmeans(self, aspect, vectors, id_sentences, k=50):
        ...
        with utils_opizer.Capturing() as output:
